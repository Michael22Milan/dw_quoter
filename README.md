...
| ⚡ **快速报价** | 输入重量、材质、难度/风险系数、后处理参数，毫秒级生成分项报价 |
...
**示例**:
- 基准打印价格 = 1000元
- 难度系数 = 1 (正常)，风险系数 = 0 → 打印价格 = 1000 × 1 = **¥1000**
- 后处理 2小时 × 50元 = **¥100**
- 最终报价 = **¥1100**
...
| 版本 | 更新内容 |
|------|----------|
| v2.2 | **报价结构细分**: 打印价格+后处理价格；难度系数改为下拉选择(1/2/3)；新增风险系数(0/0.5/1/1.5/2)；新增后处理时长和单价输入 |
